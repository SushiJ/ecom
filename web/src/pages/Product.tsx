...
export default function Product({ skipDelay = false }: { skipDelay?: boolean }) {
  const dispatch = useAppDispatch();
  const { id } = useParams<{
    id: string;
  }>();
  const { data, isError, isLoading, refetch, error } = useGetProductsByIdQuery(id!);
  ...
  return (
    <div className="min-h-full py-4">
      ...
      <Reviews isLoading={isLoading} id={id!} reviews={data.reviews} refetch={refetch} />
    </div>
  );
}
